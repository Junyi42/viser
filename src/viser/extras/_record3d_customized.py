...
class Record3dLoader_Customized:
    ...
    def __init__(self, data_dir: Path, conf_threshold: float = 1.0):
...
class Record3dFrame:
    ...
    def get_point_cloud(
        ...
        homo_grid = np.pad(grid[conf_mask * mask], np.array([[0, 0], [0, 1]]), constant_values=1)
        local_dirs = np.einsum("ij,bj->bi", np.linalg.inv(K), homo_grid)
        dirs = np.einsum("ij,bj->bi", T_world_camera[:3, :3], local_dirs)
        points = (T_world_camera[:, -1] + dirs * depth[conf_mask * mask, None]).astype(np.float32)
        point_colors = rgb[conf_mask * mask]
